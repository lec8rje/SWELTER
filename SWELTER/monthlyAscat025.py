...
## Morning or afternoon swath
        ...
        if self.doAM:
            self.suffix = '_am.gra'
            self.fidOut = Dataset( 'ascatAmMonthlyEurope.nc', 'w', format = \
                                   'NETCDF3_CLASSIC')
        else:
            self.fidOut = Dataset( 'ascatPmMonthlyEurope.nc', 'w', format = \
                              'NETCDF3_CLASSIC')
            self.suffix = '_pm.gra'
##
##
...
##    
    def writeCDF(self):
        ...
        self.fidOut.createDimension( 't', len( self.years) * len( self.months) )
        
        self.fidOut.createVariable( 'time', np.float32, ( 't') )
        self.fidOut.variables['time'].units = "Months since 01-2007"
        self.fidOut.variables['time'][:] = np.arange(len( self.years) * len( self.months) )
