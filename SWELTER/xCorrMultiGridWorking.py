...
##
    ...
    def loopOverGrid(self):
        ...
        for self.yEra in xrange( self.nYEra):
            for self.xEra in xrange( self.nXEra):


                if ( self.mask[self.yEra, self.xEra ] < 1.0):
                    continue
                if ( self.xEra < self.nXEra-1):
                    if ( self.mask[self.yEra, self.xEra+1 ] < 1.0):
                        continue
                if ( self.yEra < self.nYEra-1):
                    if ( self.mask[self.yEra+1, self.xEra ] < 1.0):
                        continue
                if ( self.yEra > 0):                    
                    if ( self.mask[self.yEra-1, self.xEra ] < 1.0):
                        continue
...
##                for self.month in xrange( 1):
        ...
        for month in xrange( len( self.months)):
            ...
            mPlot = np.ma.masked_array( np.squeeze( self.corrCoeffGrid[month,:,:]), np.squeeze( self.corrCoeffGrid[month,:,:]) < -10.0)
            plt.imshow( mPlot)
            plt.colorbar()
...
##        get horrible errors.
        if np.any( np.isreal( ( ( xIn <= 1.0) & ( np.isreal( yIn )) ) )):
            ...
            rPoints = 0
##        print( 'There are ', rPoints,' points in the calculation')
##
...
##
    def writeCorCoeffCdf(self):
            fidOut = Dataset("corCoefMonthly.nc", "w",\
...
##            print(fidOut.variables['r'][:].shape, self.corrCoeffGrid[:].shape)
    ...
    def writeSlopeCdf(self):
        fidOut = Dataset("slopeMonthly.nc", "w",\
                         format = "NETCDF3_CLASSIC")
        fidOut.createDimension( 't', len( self.months))
        fidOut.createDimension( 'y', self.nYEra)
        fidOut.createDimension( 'x', self.nXEra)
